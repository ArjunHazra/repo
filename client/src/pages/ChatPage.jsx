...
const ChatPage = () => {
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      ...
      try {
        const response = await axios.get('http://localhost:8000/api/sessions/accepted', {
  ...
  useEffect(() => {
    ...
    const socketIo = io('http://localhost:8000/sessions', {
    ...
    socketIo.on('receive_message', (data) => {
      ...
      if (data.sender && data.receiver && data.sessionId === sessionId) {
  ...
  useEffect(() => {
    const socketIoNotification = io('http://localhost:8000/notifications', {
      transports: ['websocket'],
    });
  ...
  useEffect(() => {
    if (selectedConnection) {
      const fetchMessages = async () => {
        const token = localStorage.getItem('token');
        try {
          const response = await axios.get(`http://localhost:8000/api/sessions/message/${selectedConnection._id}`, {
            headers: { 'x-auth-token': token },
          });
  ...
  const handleSendMessage = (message, file) => {
    ...
    axios.post('http://localhost:8000/api/sessions/message', formData, {
      headers: { 'x-auth-token': token },
    })
      .then((response) => {
  ...
  const handleMarkSession = async (status) => {
    try {
      ...
      await axios.post(
        'http://localhost:8000/api/sessions/mark-session',
        {
          sessionId,
          status,
          rating,
      ...
      const updatedSession = await axios.get('http://localhost:8000/api/sessions/accepted', {
        headers: {
