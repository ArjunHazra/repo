...
const ProfileSettingsPage = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (data.profilePicture) {
          setImagePreview(
            `http://localhost:8000/uploads/profile-pictures/${data.profilePicture}`
          );
        }
      } catch {
  ...
  const avatarSrc = imagePreview
    ...
    ? `http://localhost:8000/uploads/profile-pictures/${formData.profilePicture}`
    : defaultAvatar;
...
const handleUpdate = async () => {
  ...
  try {
    const token = localStorage.getItem('token');
    const res = await axios.put('http://localhost:8000/api/users/profile', payload, {
      headers: {
        'x-auth-token': token,
